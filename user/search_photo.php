<?php
  ...
  <!-- // <option value='$album_id'>$album_title</option> -->
    ...
    <form class="form-inline my-2 my-lg-0 d-flex mx-3" action="" method="get">
  ...
  $search_query="select * from photos where photo_title like '%$search_data_value%'";
  $result_query=mysqli_query($conn,$search_query);
  $num_of_rows=mysqli_num_rows($result_query);
  if($num_of_rows==0){
  echo "<h2 class='text-center text-danger'> No result can be found ! </h2>";
}
    while($row=mysqli_fetch_assoc($result_query)){
